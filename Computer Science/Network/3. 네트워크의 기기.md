...
- 로드 밸런서의 대표적인 기능: **서버 이중화**
    - 의미: 로드 밸런서는 2대 이상의 서버를 기반으로 **가상 IP**를 제공하고 이를 기반으로 안정적인 서비스 제공한다.
- 특징
...
- L7 스위치와 L4 스위치의 차이점
  - L4 스위치: **전송 계층**을 처리하는 기기로 **IP와 포트**를 기반으로 트래픽 분산한다.
...
- **부가 기능**: 기본적으로 Processor, RAM, OS를 탑재하므로 많은 부가 기능 보유한다.
